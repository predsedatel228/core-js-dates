...
function getTime(date) {
  ...
  return `${String(currDate.getHours()).padStart(2, '0')}:${String(currDate.getMinutes()).padStart(2, '0')}:${String(currDate.getSeconds()).padStart(2, '0')}`;
}
...
function getNextFriday(date) {
  ...
  if (newDate.getDay() === 5) newDate.setDate(newDate.getDate() + 1);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (objDateEnd - objDateStart) / (1000 * 60 * 60 * 24) + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  const startDate = new Date(year, parseInt(month, 10) - 1);
...
function getWeekNumberByDate(date) {
  ...
  if (firstDayOfYear === 1 || firstDayOfYear === 2 || firstDayOfYear === 4) {
    answer = Math.ceil(
      (Date.parse(date) - Date.parse(startDate)) / (1000 * 60 * 60 * 24 * 7)
    );
  } else {
    answer =
      Math.ceil(
...
function getNextFridayThe13th(date) {
  const currDate = date;
  while (!(currDate.getDate() === 13 && currDate.getDay() === 5)) {
    currDate.setDate(currDate.getDate() + 1);
  }
  return currDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const datePeriod = [];
  const answer = [];
  const currDate = startPeriod;
  while (currDate <= endPeriod) {
    datePeriod.push(
      `${String(currDate.getDate()).padStart(2, '0')}-${String(currDate.getMonth() + 1).padStart(2, '0')}-${String(currDate.getFullYear()).padStart(2, '0')}`
    );
    currDate.setDate(currDate.getDate() + 1);
  }
  for (let i = 0; i < datePeriod.length; i += countOffDays) {
    for (let t = 0; t < countWorkDays; t += 1) {
      answer.push(datePeriod[i]);
      i += 1;
    }
  }
  return answer.filter(
    (el, index) => answer.indexOf(el) === index && el !== undefined
  );
}
